...
function History() {
    ...
    const { userData, isLoading: authLoading, isReady } = useContext(UserContext);
    ...
    const fetchDiscussionRooms = useCallback(async (isLoadMore = false) => {
        ...
        try {
            console.log('🔄 Fetching discussion rooms for user:', userData._id);
            ...
            console.log('✅ Fetched discussion rooms:', result?.length || 0, 'sessions');
            ...
            if (err.message?.includes('Network')) {
                setError('Network error. Please check your Convex connection.');
            } else if (err.message?.includes('timeout')) {
                setError('Request timeout. Convex backend may be slow or unresponsive.');
            } else if (err.message?.includes('404')) {
                setError('Query not found. Please check if Convex functions are deployed.');
            } else {
    ...
    }, [convex, userData?._id, authLoading, isReady]);
