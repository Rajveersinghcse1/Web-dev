...
if (!CONFIG.GEMINI_API_KEY) {
    console.error('❌ CRITICAL: Gemini API key not found in environment variables for GlobalServices');
...
let isOnline = true; // Default to true for SSR
let pythonServerAvailable = null; // null = unknown, true/false = tested

// Initialize network status only in browser
...
// Note: Speech-to-Text is now handled directly by browser's Web Speech API
// No server-side token or API required - works offline!
...
export const exportConversationText = (conversation, topic, summary, coachingOption) => {
    ...
    if (typeof window === 'undefined' || typeof document === 'undefined') {
        console.error('Cannot export conversation on server');
...
export const createWebSpeechRecognition = (options = {}) => {
    // Only run in browser
    if (typeof window === 'undefined') {
        console.error('Cannot create Web Speech Recognition on server');
        return null;
    }
...
export const checkAllServices = async () => {
    ...
    results.webSpeech = 'webkitSpeechRecognition' in window || 'SpeechRecognition' in window;
