...
class UltraAdvancedLASParser {
    ...
    async parseWellLogLAS(uint8Array) {
        ...
        // Parse well log structure
        console.log('🔄 Parsing well log structure...');
        this.wellLogData = this.parseWellLogStructure(text);
        
        // Convert well log data to 3D point cloud
        console.log('🔄 Converting to 3D point cloud...');
        this.points = this.convertWellLogTo3D(this.wellLogData);
        this.calculateBounds();
        
        console.log(`✅ Generated ${this.points.length} 3D points`);
    ...
    splitIntoSections(text) {
        ...
        for (const line of lines) {
            ...
            } else if (trimmed.startsWith('~A') || (trimmed.includes('Depth') && trimmed.includes('Delta-T'))) {
                currentSection = 'data';
                inDataSection = true;
    ...
    parseDataSection(text, curvesSection) {
        ...
        for (const rawLine of lines) {
            ...
            if (/^[A-Za-z]/.test(trimmed) && trimmed.split(/\s+/).some(tok => /[A-Za-z]/.test(tok))) continue; // header-ish
            if (!/^[-+]?\d/.test(trimmed)) continue; // must start with number

            const values = trimmed.split(/\s+/).filter(v => v !== '');
            if (values.length === 0) continue;

            // Allow lines with fewer values; map what we have
            const dataPoint = {};
            ...
            if (sampleLogged < 5) {
                console.log('🔎 Sample parsed line ->', { raw: trimmed, mapped: dataPoint });
                sampleLogged++;
            }
        }
